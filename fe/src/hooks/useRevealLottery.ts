...
export function useRevealLottery({
  ...
  useEffect(() => {
    if (writeError) {
      const message = parseContractError(
        String(writeError.message || writeError)
      );
      setError(new Error(message));
    } else if (confirmError) {
      const message = parseContractError(
        String(confirmError.message || confirmError)
      );
      setError(new Error(message));
    } else {
      setError(null);
  ...
  const reveal = (creatorSecret: string) => {
    ...
    if (now < revealTime) {
      ...
      setError(
        new Error(
          `Reveal time not reached yet. Please wait ${minutesRemaining} more minutes`
        )
      );
      return;
    }
    ...
    try {
      ...
      const hexSecret = creatorSecret.startsWith("0x")
        ? creatorSecret
        : `0x${creatorSecret}`;

      // Validate hex format
      if (!/^0x[0-9a-fA-F]{64}$/.test(hexSecret)) {
        throw new Error("Invalid secret format: must be 32 bytes hex string");
      }

      // Call revealLottery on the contract - pass hex string directly, viem will encode it
      writeContract({
        address: address as `0x${string}`,
        abi: LOTTERY_FACTORY_ABI,
        functionName: "revealLottery",
        args: [lotteryId, hexSecret as `0x${string}`],
      });
    } catch (err) {
      setError(
        err instanceof Error ? err : new Error("Failed to reveal lottery")
      );
    }
  };
